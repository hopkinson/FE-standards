...
* 提高工作效率及协同开发的便捷性；
* 达成相对统一的代码风格；
* 提供一套HTML、CSS范式，来帮助开发人员快速做出高质量的符合要求的页面。
...
* CSS模块文件，其文件名最好与模块名一致；

css命名采用weui风格,a-b_cc形式：
...
> 那么该模块的文件名应该为：`dudu-cell.css`

* CSS页面文件，其文件名必须与HTML文件名一致；

> 假定有一个HTML页面叫 `seller.html`，那么其相对应的主CSS页面文件名应该为：`seller.css`

> 假定现在有一个 `seller.html`，里面有2个模块：

	+<section class="dudu-list">
	+<section class="dudu-info">

那么开发人员能快速找到与该页面相关的3个直接CSS文件，包括：`seller.css`, `dudu-list.css`, `dudu-info.css`
...
> 在 Less/Sass 中，如果文档中出现中文，却未显示定义编码，将会编译出错，为了统一各种写法，且提前规避错误几率，统一要求每个CSS文档都需要定义编码
...
不允许：
	...
	<a href=http://www.xxx.com class=home>dudu</a>
...
推荐：
	...
	<img src="banner.jpg" alt="我是banner alt" />
...
如果需要对模块进行注释,请在每个模块开始和结束的地方添加注释
...
代码如：
	...
	<section class="dudu-cell">
		<header class="dudu-cell_hd">
			<h1 class="title">模块标题</h1>
		</header>
		<div class="dudu-cell_bd">
			<p class="info">一些实际内容</p>
		</div>
		<footer class="dudu-cell_ft">
			<a href="#" class="more">更多</a>
		</footer>
	</section>

> 其中 `.dudu-cell_hd`, `.dudu-cell_bd`, `.dudu-cell_ft` 为可选，视具体模块情况决定是否需要抽象为这种 **头，中，尾** 的结构
...
* 命名应该由单词、中划线或数字组成；
* 不推荐使用拼音（约定俗成的除外，如：youku, souhu,baidu等），尤其是缩写的拼音、拼音与英文的混合。
...
> 我们知道2种方式都有不少支持者，但 "-" 能让你少按一次shift键，并且更符合CSS原生语法
...
js-|所有用于纯交互的命名，不涉及任何样式规则。JSer拥有全部定义权限|js-tab,js-switch
...
假定有一个模块如前文 [HTML模块化](#html-module)，那么 `dudu-cell.scss` 的写法大致如下：
...
* 现在gulp或者webpack等自动化工具有对应的插件来自动实现这一过程,大大提升了效率
